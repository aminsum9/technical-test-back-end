...
function result(sessions) {
 ...
 sessions.forEach(e => {
    if(!sessionsItems.includes(e.session_id)){
      sessionsItems.push(e.session_id);
    }
  });

  sessionsItems = sessionsItems.map(item => {
    var itemSession = sessions.find(e => e.session_id == item);

    var sessionClasses = sessions.filter(e => {
      return e.session_id == item
    });

    var classes = sessionClasses.map(item => {

      var sessionStudents = sessions.filter(e => {
        return e?.class?.class_id == item?.class?.class_id
      });

      var students = sessionStudents.map(item => {
        return { 
