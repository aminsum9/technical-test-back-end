async function UpdateCandidate(
  ...
  if (candidate_input?.school) candidate_input.school = String(candidate_input.school).toUpperCase();

  if (candidate_input?.campus) candidate_input.campus = String(candidate_input.campus).toUpperCase();

  if (candidate_input.civility) {
    switch (candidate_input?.civility) {
      case 'neutral':
  ...
  // REFACTOR START: IBAN Validation for Parent
  var parentsToValidate = (candidate_input?.parents ?? []).filter(e => {
    return parent.iban && parent.bic && parent.account_holder_name;
  });

  parentsToValidate.forEach(async (parent) => {
  ...
  // REFACTOR END: IBAN Validation for Student

  // REFACTOR START: IBAN Validation for Payment Support
  ...
  // REFACTOR END: IBAN Validation for Payment Support

  // REFACTOR START: Handle Save IBAN History
  ...
  if (candidateBeforeUpdate) {
    ...
    if (candidateBeforeUpdate?.payment_supports?.length && candidate_input.payment_supports?.length) {
    ...
    if (candidateBeforeUpdate?.parents?.length && candidate_input?.parents?.length) {
      ...
      for (const parentBeforeUpdate of candidateBeforeUpdate.parents) {
        ...
        let parentIbanData = candidate_input.parents.find((parent) => String(parentBeforeUpdate._id) === String(parent._id) && parent.iban !== parentBeforeUpdate.iban);
  ...
  if (!candidate_input?.legal_representative?.unique_id) {
    candidate_input.legal_representative.unique_id = oldCandidate.legal_representative.unique_id
  ...
  const parentalLink = candidate_input?.legal_representative?.parental_link ?? '';
  if (!candidate_input?.legal_representative?.civility && parentalLink) {
  ...
  var legalReprevLastName = candidate_input?.legal_representative?.last_name;
  if (legalReprevLastName) {
    candidate_input.legal_representative.last_name = legalReprevLastName.toUpperCase();
  }
  // REFACTOR END: Serialize Legal Representative

  // REFACTOR START: Serialize candidate input finance
  ...
  if (!candidate_input?.finance && !oldCandidate?.finance && oldCandidate?.selected_payment_plan?.payment_mode_id) {
    candidate_input.finance = (candidate_input?.payment_supports?.length || oldCandidate?.payment_supports?.length) ? 'family' : 'my_self';
  }
  ...
  // REFACTOR END: Serialize candidate input finance

  // REFACTOR START: Candidate Email Validation
  // ************** add condition if candidate old email is different from input and candidate is registered
  var checkCandidateEmail =((oldCandidate.email && candidate_input.email && oldCandidate.email !== candidate_input.email) ||
      (!oldCandidate.email && candidate_input.email));

  if (oldCandidate?.user_id && oldCandidate?.candidate_admission_status === 'registered' && candidate_input && checkCandidateEmail) {
    // ************** if different, then remove the recovery code in user
    await UserModel.updateOne({ _id: oldCandidate.user_id },
      {
        $set: {
  ...
  // REFACTOR END: Candidate Email Validation

  // REFACTOR START: Check Payment Plan
  if (
    candidate_input && (oldSelectedPaymentPlanData?.total_amount ?? 0) > 0 &&
  ...
  // REFACTOR END: Check Payment Plan

  if (!userId) userId = oldCandidate.user_id; //in case this mutation called without auth token

  // REFACTOR START: Check Candidate Contract
  var candidateContractStatus = ['registered', 'engaged', 'resigned_after_engaged', 'resigned_after_registered'];
  var checkCaandidatePaymentMethod = (candidate_input.payment_method && candidate_input.payment_method !== oldCandidate.payment_method)

  if (!oldCandidate.admission_process_id && 
    (is_from_admission_form || checkCaandidatePaymentMethod) && 
    candidateContractStatus.includes(oldCandidate.candidate_admission_status)) {
      await CandidateUtility.validateCandidateInput(candidate_input, oldCandidate);

      if (!candidate_input.payment_method && await CandidateUtility.getCandidateCurrentStep(oldCandidate) !== 'down_payment') {
        throw new ApolloError('Cannot edit data, candidate already signed school contract!');
      }
    }
  }
  // REFACTOR END: Check Candidate Contract

  // REFACTOR START: Update Candidate Status
  // To update candidate status, and validation on readmission assignment table

  var candidate_admission_status = ['resigned','resigned_after_engaged','resigned_after_registered','no_show','resignation_missing_prerequisites','resign_after_school_begins','report_inscription'];

  if (candidate_input.candidate_admission_status &&
      oldCandidate.candidate_admission_status === 'registered' &&
      candidate_admission_status.includes(candidate_input.candidate_admission_status)) {
    ...
    let candidateIds = [];
    if (otherCandidateSameStudent.length === 0) candidateIds.push(oldCandidate._id); else 
  ...
  var oldCandidateAdmissionStatus = ['resigned_after_engaged','resigned_after_registered','no_show','resignation_missing_prerequisites','resign_after_school_begins','report_inscription'];

  if (
    (candidate_input.candidate_admission_status &&
      oldCandidateAdmissionStatus.includes(oldCandidate.candidate_admission_status) &&
  ...
  // REFACTOR END: Update Candidate Status
  ...
  // REFACTOR START: Process Change Step
  if (oldCandidate.admission_process_id) {
    ...
    if (candidate_input.payment_method === 'cash' && admissionProcess?.steps?.length) {
  ...
  // REFACTOR START: Process Change Step Down Payment 
  // process to accept step down_payment_mode
  // Accept step down_payment_mode if FE send payment 'no_down_payment'
  var checkTypeOfFormation = typeOfFormation && (continuousTypeOfFormation.includes(typeOfFormation.type_of_formation) || oldCandidate.readmission_status === 'readmission_table');
  var checkPaymentMethod =  candidate_input.payment_method &&  candidate_input.payment_method !== oldCandidate.payment_method;

  if ((checkTypeOfFormation && checkPaymentMethod 
    ...
    if (admissionProcess?.steps?.length) {
      ...
      if (downPaymentStep) {
        ...
        if (typeOfFormation.type_of_formation === 'classic') await CandidateUtility.proceedRegisteredStudent(_id, lang);
      }
    }
  }
  // REFACTOR END: Process Change Step Down Payment 

  // REFACTOR START: Process Change Step Campus Validation 
  ...
  // REFACTOR END: Process Change Step Campus Validation 

  // REFACTOR START: Process Accept Step School Contract 
  ...
  // REFACTOR END: Process Accept Step School Contract 
  // REFACTOR END: Process Change Step


  // REFACTOR START: Candidate Input Payment Validation
  if (['check', 'transfer'].includes(candidate_input?.payment_method) && oldCandidate?.payment === 'not_authorized') candidate_input.payment = 'not_done';

  if (oldCandidate.payment === 'done' && candidate_input.payment === 'pending') candidate_input.payment = oldCandidate.payment;

  if (oldCandidate.payment_method === candidate_input?.payment_method) candidate_input.payment = oldCandidate.payment;
  ...
  if (candidate_input.finance && oldCandidate.finance !== candidate_input.finance && candidate_input.finance === 'my_self') {
    ...
    if (oldCandidate.method_of_payment === 'sepa' && !is_save_identity_student) {
      if (!candidate_input.iban || !candidate_input.bic || !candidate_input.account_holder_name) throw new ApolloError('Answer of question is required');

      const checkIban = await IbanHistoryModel.findOne({ candidate_id: oldCandidate._id }).sort({ _id: -1 }).lean();

      if (checkIban?.message !== 'success')  throw new ApolloError('IBAN not verified');
    }
  }

  // *************** failsafe, empty parent if required data is null
  if (candidate_input?.parents?.length) {
    const validParentsData = candidate_input.parents.filter(candidate_input => {
      return candidate_input?.parents?.family_name && candidate_input?.parents?.name && candidate_input?.parents?.email;
    })
  ...
  if (candidate_input?.payment_supports?.length) {
    const validatedPaymentSupportsData = candidate_input.payment_supports.filter(candidate_input => {
      return  candidate_input.payment_supports?.family_name &&
        candidate_input.payment_supports?.name &&
        candidate_input.payment_supports?.email;
    })

    candidate_input.payment_supports = validatedPaymentSupportsData;
  }
  // REFACTOR END: Candidate Input Payment Validation

  // ******************* Save history legal representative
  await CandidateUtility.SaveHistoryLegalRepresentative(candidate_input, _id, userId);

  // REFACTOR START: Process Question
  ...
  if (candidate_input.cvec_number || candidate_input.ine_number) {
    if (candidate_input.cvec_number) andidate_input.cvec_number = candidate_input.cvec_number.toUpperCase();

    if (candidate_input.ine_number) candidate_input.ine_number = candidate_input.ine_number.toUpperCase();
